...
class CommonLoader extends Component {
  constructor(props) {
    super(props);
    this.state = {
      modalVisibilty: this.props.isLoading || false,
      apiname: ''
    }
  }

  componentWillReceiveProps(nextProps) {
    this.setState({ modalVisibilty: nextProps.loadingState || nextProps.isLoading });
    // for experimentational purposes, dont load for 10 seconds
    if (nextProps.loadingState && !this.props.loadingState && !nextProps.noAutoOff)
...
function mapStateToProps(state) {
  ...
  return { theme, apiname, loadingState: state.app.loading };
}
